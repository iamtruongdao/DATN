import { deleteTagApi, getAllTagApi } from '@/apis/tag.api'
import { DataTable } from '@/components/Table'
import { TableToolbar } from '@/components/Table/TableToolBar'
import { Skeleton } from '@/components/ui/skeleton'
import { getColumn } from '@/pages/Admin/TagPage/column'
...
const TagPage = () => {
  ...
  const handleEdit = (tag: Tag) => {
    navigate('/admin/tag-edit', { state: tag })
  }
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await getAllTagApi(queryParams)
      if (response.code === 0) {
