import { deletePostApi, getPostFilter } from '@/apis/post.api'
import { DataTable } from '@/components/Table'
import { TableToolbar } from '@/components/Table/TableToolBar'
import { Skeleton } from '@/components/ui/skeleton'
import { getColumn } from '@/pages/Admin/PostPage/column'
...
const PostPage = () => {
  ...
  const handleViewDetail = async (post: Post) => {
    try {
      ...
      setSelectedPost(post) // Thay vì setSelectedPost
      setShowDetail(true)
    } catch (error) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await getPostFilter(queryParams)
      if (response.code === 0) {
  ...
  return (
    <div className='space-y-2'>
      ...
      />
      <PostDetailSheet post={selectedPost} open={showDetail} onOpenChange={setShowDetail} />
      <div className='p-2'>
